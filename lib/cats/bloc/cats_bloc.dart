...
class CatsBloc extends Bloc<CatsEvent, CatsState> {
  CatsBloc() : super(CatsInitial()) {
    on<GetLastCat>((event, emit) async {
      try {
        emit(CatsInitial());
    ...
    on<GetRandomCat>((event, emit) async {
      try {
        emit(CatsLoading());
        final box = await _openHiveBox();
        ...
        final factRequest = await ApiClient(
                Dio(BaseOptions(headers: {
                  ...
                  'X-CSRF-TOKEN': 'pNAXkRUxs7fXbiUHOBgE25UuMTcSVSgLxeny4DMJ'
                }, contentType: 'application/json')),
                baseUrl: 'https://catfact.ninja')
            .getCatFacts();
        String date = await _getCatDate();
        CatModel cat = imageRequest.add(fact: factRequest.fact).add(date: date);
        List<CatModel> cats = _getCatsFromBox(box);
        cats.add(cat);
        await box.put(
            'cats', jsonEncode(cats.map((cat) => cat.toJson()).toList()));
        emit(CatsLoaded(cat: cat));
      } catch (e) {
        emit(CatsError(message: e.toString()));
      }
    });
    on<GetCatsHistory>((event, emit) async {
      try {
        emit(HistoryInitial());
        final box = await _openHiveBox();
        List<CatModel> cats = _getCatsFromBox(box);
        emit(CatsHistoryLoaded(cats: cats));
      } catch (e) {
        emit(CatsError(message: e.toString()));
      }
    });
  ...
  Future<String> _getCatDate() async {
    ...
    return DateFormat.yMMMMd(systemLocale).format(DateTime.now());
  }
}
